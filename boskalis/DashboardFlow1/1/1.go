...
// Document-Name: DashboardFlow1
// Version: 1
// Type or Namespace: _DashboardFlow1
// Dsl: 1

type _DashboardFlow1 struct {
...
func New_DashboardFlow1(workflowIOManager dependencies.WorkflowIOManager) *_DashboardFlow1 {
	return &_DashboardFlow1{
		dataMap:           map[string]interface{}{},
		EventMap:          map[string]Event{},
...
func (w *_DashboardFlow1) trackTaskComplete(taskName string) {
	timestamp := time.Now().Format(time.RFC3339)
	w.taskHistory[taskName] = timestamp
	w.logger.Info("Task completed: %s", taskName)
}

func (w *_DashboardFlow1) Execute(ctx workflow.Context, input interface{}) (string, error) {
	...
	if err := workflow.SetQueryHandler(ctx, "getCurrentTask", func() (string, error) {
		...
		w.logger.Error("failed to register query handler : _DashboardFlow1", err)
		return "", err
	}
...
func (w *_DashboardFlow1) PersistActualInput(ctx workflow.Context, data interface{}) (string, error) {
...
func (w *_DashboardFlow1) handleActivityRetryLoop(ctx workflow.Context, activityName, taskName string, execute func() (interface{}, error), initialError error) (interface{}, error) {
...
func (w *_DashboardFlow1) produceWorkflowTaskStart(ctx workflow.Context, taskName string, input map[string]interface{}) error {
...
func (w *_DashboardFlow1) produceWorkflowTaskCompletion(ctx workflow.Context, taskName string, outputMap map[string]interface{}) error {
	// Get workflow execution info
	info := workflow.GetInfo(ctx)
	workflowID := info.WorkflowExecution.ID
	runID := info.WorkflowExecution.RunID
...
func (w *_DashboardFlow1) produceWorkflowCompletion(ctx workflow.Context, outputMap map[string]interface{}) error {
	info := workflow.GetInfo(ctx)
	workflowID := info.WorkflowExecution.ID
	runID := info.WorkflowExecution.RunID
...
func (w *_DashboardFlow1) safeExecuteActivity(ctx workflow.Context, currentTask string, activityName string, input interface{}) (interface{}, error) {
	var result interface{}
	retryCount := 0
	ctx = workflow.WithActivityOptions(ctx, workflow.ActivityOptions{
		ActivityID: currentTask + "_retry_" + strconv.Itoa(retryCount),
	})
	retryCount++
	// Execute activity
...
func (w *_DashboardFlow1) File_Copy1(ctx workflow.Context, input interface{}) error {
	...
	w.dataMap["activity-id"] = "988f0039-8e4d-412b-bf74-7761dab3c3df"
	dataId, err := w.PersistActualInput(ctx, w.dataMap)
	if err != nil {
	...
	_, err = w.safeExecuteActivity(ctx, w.currentTask, "FileCopyProxyTest", map[string]interface{}{"data_id": dataId})
	if err != nil {
		w.logger.Error("Workflow terminated during activity execution", zap.Error(err))
		return err
	}
	err = w.CustomUpdateHandler(ctx, "FileCopyProxyTest")
	if err != nil {
		return err
	}
...
func (w *_DashboardFlow1) waitEvent(ctx workflow.Context, input interface{}, taskName string, EventType string) error {
...
func (w *_DashboardFlow1) CustomUpdateHandler(ctx workflow.Context, updateName string) error {
...
func (w *_DashboardFlow1) Pgp_Decrypter1(ctx workflow.Context, input interface{}) error {
	w.previousTask = w.currentTask
	...
	w.dataMap["activity-id"] = "44023120-3064-4f3e-a4d8-0ba059f121a9"
	dataId, err := w.PersistActualInput(ctx, w.dataMap)
	if err != nil {
		w.logger.Error("Error persisting actual input: %%s", err)
		return err
	...
	_, err = w.safeExecuteActivity(ctx, w.currentTask, "PGPDecryptorProxyTest", map[string]interface{}{"data_id": dataId})
	if err != nil {
		w.logger.Error("Workflow terminated during activity execution", zap.Error(err))
		return err
	}
	err = w.CustomUpdateHandler(ctx, "PGPDecryptorProxyTest")
	if err != nil {
		return err
	}
...
func (w *_DashboardFlow1) Transformer_M1(ctx workflow.Context, input interface{}) error {
	w.previousTask = w.currentTask
	...
	w.dataMap["activity-id"] = "dbcc9b18-7d68-4167-a258-8b9681bd1afa"
	...
	_, err = w.safeExecuteActivity(ctx, w.currentTask, "TransformerProxyTest", map[string]interface{}{"data_id": dataId})
	if err != nil {
		w.logger.Error("Workflow terminated during activity execution", zap.Error(err))
		return err
	}
	err = w.CustomUpdateHandler(ctx, "TransformerProxyTest")
	if err != nil {
		return err
	}
...
func (w *_DashboardFlow1) exit(ctx workflow.Context) {
