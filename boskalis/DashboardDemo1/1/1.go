...
// Document-Name: DashboardDemo1
// Version: 1
// Type or Namespace: _DashboardDemo1
// Dsl: 1

type _DashboardDemo1 struct {
...
func New_DashboardDemo1(workflowIOManager dependencies.WorkflowIOManager) *_DashboardDemo1 {
	return &_DashboardDemo1{
		dataMap:           map[string]interface{}{},
		EventMap:          map[string]Event{},
...
func (w *_DashboardDemo1) trackTaskComplete(taskName string) {
	timestamp := time.Now().Format(time.RFC3339)
	w.taskHistory[taskName] = timestamp
	w.logger.Info("Task completed: %s", taskName)
}

func (w *_DashboardDemo1) Execute(ctx workflow.Context, input interface{}) (string, error) {
	...
	if err := workflow.SetQueryHandler(ctx, "getCurrentTask", func() (string, error) {
		...
		w.logger.Error("failed to register query handler : _DashboardDemo1", err)
		return "", err
	}
	...
	// Execute first task
	if err := w.Transformer(ctx, w.dataMap); err != nil {
		return "", err
	}
...
func (w *_DashboardDemo1) PersistActualInput(ctx workflow.Context, data interface{}) (string, error) {
...
func (w *_DashboardDemo1) handleActivityRetryLoop(ctx workflow.Context, activityName, taskName string, execute func() (interface{}, error), initialError error) (interface{}, error) {
...
func (w *_DashboardDemo1) produceWorkflowTaskStart(ctx workflow.Context, taskName string, input map[string]interface{}) error {
...
func (w *_DashboardDemo1) produceWorkflowTaskCompletion(ctx workflow.Context, taskName string, outputMap map[string]interface{}) error {
	// Get workflow execution info
	info := workflow.GetInfo(ctx)
	workflowID := info.WorkflowExecution.ID
	runID := info.WorkflowExecution.RunID
...
func (w *_DashboardDemo1) safeExecuteActivity(ctx workflow.Context, activityName string, input interface{}) (interface{}, error) {
...
func (w *_DashboardDemo1) Transformer(ctx workflow.Context, input interface{}) error {
	...
	w.dataMap["activity-id"] = "63f2da29-b982-45f9-97fa-678d9757856e"
	...
	err = w.waitEvent(ctx, input, "Transformer", "file_copy.com")
	if err != nil {
		return err
	}
...
func (w *_DashboardDemo1) waitEvent(ctx workflow.Context, input interface{}, taskName string, EventType string) error {
...
func (w *_DashboardDemo1) CustomUpdateHandler(ctx workflow.Context, updateName string) error {
...
func (w *_DashboardDemo1) Pgp_Decrypter1(ctx workflow.Context, input interface{}) error {
	w.previousTask = w.currentTask
	...
	w.dataMap["activity-id"] = "4f41aba4-bc0f-4b14-bd33-04e538ffe050"
...
func (w *_DashboardDemo1) Transformer_M1(ctx workflow.Context, input interface{}) error {
	w.previousTask = w.currentTask
	...
	w.dataMap["transformercore_in_file_path"] = w.EventMap["Pgp_Decrypter1"].Result["pgp_decryption.out.file_path"]
	w.dataMap["activity-id"] = "f89dc290-9c4e-4177-8876-6f1379c03b3e"
	w.dataMap["transformercore_in_file_names"] = w.EventMap["Pgp_Decrypter1"].Result["pgp_decryption.out.file_names"]
	dataId, err := w.PersistActualInput(ctx, w.dataMap)
	if err != nil {
		w.logger.Error("Error persisting actual input: %%s", err)
		return err
