...
// Document-Name: DashboardFlow5
// Version: 1
// Type or Namespace: _DashboardFlow5
// Dsl: 1

type _DashboardFlow5 struct {
...
func New_DashboardFlow5(workflowIOManager dependencies.WorkflowIOManager) *_DashboardFlow5 {
	return &_DashboardFlow5{
		dataMap:           map[string]interface{}{},
		EventMap:          map[string]Event{},
...
func (w *_DashboardFlow5) trackTaskComplete(taskName string) {
	timestamp := time.Now().Format(time.RFC3339)
	w.taskHistory[taskName] = timestamp
	w.logger.Info("Task completed: %s", taskName)
}

func (w *_DashboardFlow5) Execute(ctx workflow.Context, input interface{}) (string, error) {
	...
	if err := workflow.SetQueryHandler(ctx, "getCurrentTask", func() (string, error) {
		...
		w.logger.Error("failed to register query handler : _DashboardFlow5", err)
		return "", err
	}
	...
	w.dataMap["workflow_id"] = workflow.GetInfo(ctx).WorkflowExecution.ID
	w.dataMap["run_id"] = workflow.GetInfo(ctx).WorkflowExecution.RunID
	w.dataMap["tenant_id"] = workflow.GetInfo(ctx).Namespace
	w.dataMap["workflow_name"] = workflow.GetInfo(ctx).WorkflowType.Name

	// Store input in workflow data map
	w.EventMap["Trigger"] = Event{
		EventID:   "Trigger",
		EventType: "Trigger",
		Name:      "Trigger",
		Result:    inputMap,
...
func (w *_DashboardFlow5) PersistActualInput(ctx workflow.Context, data interface{}) (string, error) {
...
func (w *_DashboardFlow5) handleActivityRetryLoop(ctx workflow.Context, activityName, taskName string, execute func() (interface{}, error), initialError error) (interface{}, error) {
	...
	for {
		...
		if err == nil {
			...
			if resultMap, ok := result.(map[string]interface{}); ok {
				event := w.EventMap[taskName]
				event.Result = resultMap
				w.EventMap[taskName] = event
			}
			return result, nil
		}
...
func (w *_DashboardFlow5) produceWorkflowTaskStart(ctx workflow.Context, taskName string, inputMap map[string]interface{}) error {
	// Get workflow execution info
	info := workflow.GetInfo(ctx)
	workflowID := info.WorkflowExecution.ID
	runID := info.WorkflowExecution.RunID
	// Create the event payload
	for key, value := range inputMap["input"].(map[string]interface{}) {
		inputMap[key] = value
	}
	//delete the inputMap["input"]
...
func (w *_DashboardFlow5) produceWorkflowTaskCompletion(ctx workflow.Context, taskName string, outputMap map[string]interface{}) error {
	// Get workflow execution info
	info := workflow.GetInfo(ctx)
	workflowID := info.WorkflowExecution.ID
	runID := info.WorkflowExecution.RunID
	//log output map with task name
...
func (w *_DashboardFlow5) produceWorkflowCompletion(ctx workflow.Context, outputMap map[string]interface{}) error {
	info := workflow.GetInfo(ctx)
	workflowID := info.WorkflowExecution.ID
	runID := info.WorkflowExecution.RunID
...
func (w *_DashboardFlow5) safeExecuteActivity(ctx workflow.Context, activityName string, input interface{}) (interface{}, error) {
	var result interface{}
	currentTask := w.currentTask

	w.activityAttempts[currentTask] = w.activityAttempts[currentTask] + 1
	attemptNumber := w.activityAttempts[currentTask]

	activityCtx := workflow.WithActivityOptions(ctx, workflow.ActivityOptions{
		ActivityID:             fmt.Sprintf("%s", currentTask),
	...
	inputMap["attempt_number"] = attemptNumber
	inputMap["attempt_time"] = workflow.Now(activityCtx).Format(time.RFC3339)
	...
	for {
		...
		activityCtx = workflow.WithActivityOptions(ctx, workflow.ActivityOptions{
			ActivityID:             fmt.Sprintf("%s_%d", currentTask, attemptNumber),
			ScheduleToCloseTimeout: 120 * time.Second,
		})
		...
		attemptNumber := w.activityAttempts[currentTask]
		inputMap["attempt_number"] = attemptNumber
		inputMap["attempt_time"] = workflow.Now(activityCtx)
		...
		if activityError == nil {
			...
			if resultMap, ok := result.(map[string]interface{}); ok {
				event := w.EventMap[activityName]
				event.Result = resultMap
				w.EventMap[activityName] = event
			}
			return result, nil
		}
...
func (w *_DashboardFlow5) File_Copy1(ctx workflow.Context, input interface{}) error {
	...
	inputMap["activity-id"] = "cdfa8799-9744-42e6-b9c5-71eca805a0b2"
	inputMap["filecopy_in_file_names"] = w.EventMap["Trigger"].Result["file_listener.out.file_names"]
	inputMap["filecopy_in_file_path"] = w.EventMap["Trigger"].Result["file_listener.out.file_path"]
	w.dataMap["input"] = inputMap
	dataId, err := w.PersistActualInput(ctx, w.dataMap)
	if err != nil {
		w.logger.Error("Error persisting actual input: %%s", err)
		return err
	}
	err = w.produceWorkflowTaskStart(ctx, w.currentTask, dataMap)
	if err != nil {
	...
	outputMap := w.EventMap[w.currentTask].Result
...
func (w *_DashboardFlow5) waitEvent(ctx workflow.Context, input interface{}, taskName string, EventType string) error {
...
func (w *_DashboardFlow5) CustomUpdateHandler(ctx workflow.Context, updateName string) error {
...
func (w *_DashboardFlow5) Pgp_Decrypter1(ctx workflow.Context, input interface{}) error {
	w.previousTask = w.currentTask
	...
	inputMap := make(map[string]interface{})
	inputMap["activity-id"] = "dd8d538b-ce65-4285-9082-cfa6962841f0"
	inputMap["pgpdecryptor_in_file_path"] = w.EventMap["File_Copy1"].Result["file_copy.out.file_path"]
	inputMap["pgpdecryptor_in_file_names"] = w.EventMap["File_Copy1"].Result["file_copy.out.file_names"]
	w.dataMap["input"] = inputMap
	dataId, err := w.PersistActualInput(ctx, w.dataMap)
	if err != nil {
		w.logger.Error("Error persisting actual input: %%s", err)
...
func (w *_DashboardFlow5) Transformer_M1(ctx workflow.Context, input interface{}) error {
	w.previousTask = w.currentTask
	...
	inputMap := make(map[string]interface{})
	inputMap["activity-id"] = "9015eb75-9d1d-4352-bfb6-8c75e9b79eec"
	inputMap["transformerm_in_file_names"] = w.EventMap["Pgp_Decrypter1"].Result["pgp_decryption.out.file_names"]
	inputMap["transformerm_in_file_path"] = w.EventMap["Pgp_Decrypter1"].Result["pgp_decryption.out.file_path"]
	w.dataMap["input"] = inputMap
...
func (w *_DashboardFlow5) exit(ctx workflow.Context) {
	w.currentTask = "All tasks Executed"
	w.logger.Info("Workflow completed\n")
	err := w.produceWorkflowCompletion(ctx, w.dataMap)
	if err != nil {
